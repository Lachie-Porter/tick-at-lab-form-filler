(function () {
  ...
  if (!DATA) {
    console.error('[bm] No CSV data in localStorage. Run load-csv bookmarklet first.');
  ...
  if (!state) {
    ...
    if (!pick) { console.log('[bm] User cancelled procedure selection.'); return; }
    ...
    console.log('[bm] Selected procedure:', expName, '— entries:', DATA[expName].length);
  }
  ...
  function fillNext(form) {
    ...
    console.log('[bm] Filling entry', state.index + 1, '/', list.length, '—', person.name, '/', person.competency);
  ...
  if (window.jQuery) {
    console.log('[bm] jQuery already present, version', window.jQuery.fn.jquery);
    run(window.jQuery);
  } else {
